...
This application implements multiple layers of security to protect data integrity and prevent common web vulnerabilities.

### 1. Environment-Based Secret Key Management
The Flask secret key — used to cryptographically sign session cookies — is stored in a `.env` file rather than hardcoded in the source code. This prevents accidental exposure of credentials through version control. The `.env` file is excluded from Git via `.gitignore` and will never be pushed to GitHub.

### 2. CSRF Protection (Cross-Site Request Forgery)
Every HTML form in the application is protected by a CSRF token generated by Flask-WTF. A Cross-Site Request Forgery attack tricks a logged-in user into unknowingly submitting a malicious request. CSRF tokens prevent this by verifying that every form submission originates from the application itself, not from an external source. AJAX endpoints that use JSON (not forms) are appropriately exempted.

### 3. File Upload Validation
All uploaded files are validated through multiple checks before being saved to disk:
- **Extension check** — Only `.mcap` files are accepted
- **File size limit** — Maximum 500 MB per upload
- **Empty file check** — Zero-byte files are rejected
- **Magic byte verification** — The file header is inspected to confirm it is a genuine MCAP file, preventing renamed files of other types from being processed

### 4. HTTP Security Headers
The application sets HTTP security headers on every response using Flask-Talisman:

| Header | Value | Purpose |
|--------|-------|---------|
| X-Content-Type-Options | nosniff | Prevents the browser from guessing file types, reducing MIME-type confusion attacks |
| X-Frame-Options | DENY | Blocks the application from being embedded in an iframe, preventing clickjacking attacks |
| Content-Security-Policy | Restricted allow-list | Controls which external sources (scripts, styles, fonts) the browser is permitted to load, blocking unauthorized injected content |
| Referrer-Policy | strict-origin-when-cross-origin | Limits how much URL information is shared when navigating between pages |

### 5. Local SSL (HTTPS)
The application runs over HTTPS using a self-signed certificate generated by pyOpenSSL. HTTPS encrypts all traffic between the browser and the local server, preventing data from being read in plaintext on the network. As the certificate is self-signed (not issued by a trusted Certificate Authority), the browser will display a security warning on first access — this is expected behaviour for local development environments and can be bypassed by clicking Advanced → Proceed.

> **Note:** Session cookies are configured with `HttpOnly=True` (inaccessible to JavaScript) and `Secure=True` (transmitted over HTTPS only), further protecting user session data.

### 6. Input Sanitization
All user-submitted form inputs are sanitized before being processed or stored. This protects against two common web attacks:

**Cross-Site Scripting (XSS)** — An attacker submits HTML or JavaScript code through a form field (e.g. a technician name containing `<script>` tags). Without sanitization, this code could execute in other users' browsers when the value is displayed. All inputs are stripped of HTML tags using the `bleach` library before storage.

**SQL Injection** — An attacker submits database commands through a form field in an attempt to manipulate or destroy data. All 59 database queries in the application were audited: 58 use parameterized statements (`?` placeholders), and the one exception — the table name in `get_count()`, which SQL can only parameterize as a value, not as an identifier — is instead validated against a fixed whitelist of table names. In every case, user input is always treated as data, never as an executable command.

Uploaded filenames are also sanitized using `werkzeug.utils.secure_filename` combined with a custom `sanitize_filename()` function that removes dangerous characters and enforces a maximum filename length.

### 7. Rate Limiting
Sensitive endpoints are protected by request rate limits using Flask-Limiter. This prevents abuse such as flooding the server with repeated upload or report generation requests (Denial of Service).

| Endpoint | Limit |
|----------|-------|
| Upload MCAP | 10 requests per minute |
| Generate Report | 10 requests per minute |
| Re-Extract MCAP | 20 requests per minute |
| Add Technician | 20 requests per minute |
| Upload Screenshot | 30 requests per minute |
| Read-only pages (Home, Reports, Audit Log) | No limit |

When a limit is exceeded, the server returns HTTP 429 (Too Many Requests) with a user-friendly message asking the user to wait before retrying. Read-only pages are exempt from rate limiting as they do not modify data or consume significant resources.
